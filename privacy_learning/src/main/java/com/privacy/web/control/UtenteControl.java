...
public class UtenteControl {
	...
	/* pagina studio lezione singola */
	@GetMapping("/profilo/lezioni-private/{email}/{titolo}")
	public String progresso(@PathVariable String email, @PathVariable String titolo,
			@ModelAttribute("user") Utente user, HttpServletRequest request, HttpServletResponse response, Model model,
			HttpSession userSession) throws Exception {

		Utente u = utServ.findUtenteByEmail(email);
		userSession.setAttribute("userSession", u);

		ArgomentoStudio arg_dastudiare = argServ.findArgomentoByTitolo(titolo);
		if(arg_dastudiare != null) {
		model.addAttribute("argoment", arg_dastudiare);
		System.out.println("argomento da studiare:" + arg_dastudiare.getTitolo());}
		return "studiolezione";
	}

	/* conferma della lezione studiata */
	...
	public String lezione_studiata(Model model, @PathVariable String email, @PathVariable String titolo,
		...
		// prendo tutti i progressi dell'utente
		List<ProgressoStudio> allprog = progServ.findByEmail(email);
		model.addAttribute("allprog", allprog);

		// mi prendo l'argomento che ha appena studiato
		ArgomentoStudio argStudiato = argServ.findArgomentoByTitolo(titolo);
		model.addAttribute("arg", argStudiato);

		// setto nel progresso che ho studiato quell'argomento (titolo)
		...
		return "redirect:/users/profilo/lezioni-private/{email}";
	}
